import { ImageResponse } from 'next/og'
import { createClient } from '@supabase/supabase-js'


export const alt = 'Host Profile'
export const size = {
    width: 1200,
    height: 675,
}
export const contentType = 'image/png'

export default async function Image({ params }: { params: Promise<{ slug: string }> }) {
    const { slug } = await params
    // Assuming slug format: name-id, extract last part as ID. 
    // If simply ID, this still works if no hyphen.
    // Ideally we should use regex or consistent logic.
    const parts = slug.split('-')
    const hostId = parts[parts.length - 1] // Last part is ID

    // Initialize Supabase Client
    const supabase = createClient(
        process.env.NEXT_PUBLIC_SUPABASE_URL!,
        process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!
    )

    const { data: profile } = await supabase
        .from('profiles')
        .select('full_name, avatar_url, bio, category_id, categories(name, icon)')
        .eq('id', hostId)
        .single()

    const name = profile?.full_name || 'Tripzeo Host'
    const bio = profile?.bio ? profile.bio.slice(0, 100) + (profile.bio.length > 100 ? '...' : '') : 'Experiences Host on Tripzeo'
    // @ts-ignore
    let avatar = profile?.avatar_url || profile?.categories?.icon
    // @ts-ignore
    const category = profile?.categories?.name || 'Local Expert'

    // Ensure absolute URL if it is a relative path
    if (avatar && !avatar.startsWith('http')) {
        const baseUrl = process.env.NEXT_PUBLIC_APP_URL || 'https://www.tripzeo.com'
        avatar = `${baseUrl}${avatar.startsWith('/') ? '' : '/'}${avatar}`
    }

    return new ImageResponse(
        (
            <div
                style={{
                    width: '100%',
                    height: '100%',
                    display: 'flex',
                    flexDirection: 'column',
                    alignItems: 'center',
                    justifyContent: 'center',
                    background: '#0a0a0a', // Darker background
                    position: 'relative',
                }}
            >
                {/* Background Pattern/Gradient */}
                <div
                    style={{
                        position: 'absolute',
                        top: 0, left: 0, right: 0, bottom: 0,
                        background: 'radial-gradient(circle at 50% 0%, #1a1a1a 0%, #000 100%)',
                    }}
                />

                {/* Subtle Accent Glows */}
                <div
                    style={{
                        position: 'absolute',
                        top: '-20%',
                        left: '0%',
                        width: '100%',
                        height: '60%',
                        background: 'radial-gradient(ellipse at center, rgba(248, 80, 50, 0.15), transparent 70%)',
                        filter: 'blur(80px)',
                    }}
                />

                {/* Content Container */}
                <div
                    style={{
                        display: 'flex',
                        flexDirection: 'column',
                        alignItems: 'center',
                        justifyContent: 'center',
                        zIndex: 10,
                        width: '100%',
                    }}
                >
                    {/* Avatar Container */}
                    <div
                        style={{
                            display: 'flex',
                            position: 'relative',
                            marginBottom: 40,
                        }}
                    >
                        {/* Glow Effect */}
                        <div style={{
                            position: 'absolute',
                            top: -4, left: -4, right: -4, bottom: -4,
                            borderRadius: 1000,
                            background: 'linear-gradient(45deg, #F85032, #ff9a9e)',
                            opacity: 0.8,
                            filter: 'blur(10px)',
                        }} />

                        {avatar ? (
                            <img
                                src={`https://wsrv.nl/?url=${encodeURIComponent(avatar)}&w=300&h=300&fit=cover&output=jpg`}
                                alt={name}
                                width="240"
                                height="240"
                                style={{
                                    width: '240px',
                                    height: '240px',
                                    borderRadius: 1000,
                                    objectFit: 'cover',
                                    border: '4px solid #000',
                                    position: 'relative',
                                }}
                            />
                        ) : (
                            <div
                                style={{
                                    width: '240px',
                                    height: '240px',
                                    borderRadius: 1000,
                                    background: '#1a1a1a',
                                    display: 'flex',
                                    alignItems: 'center',
                                    justifyContent: 'center',
                                    color: '#F85032',
                                    fontSize: 100,
                                    fontWeight: 900,
                                    border: '4px solid #000',
                                    position: 'relative',
                                }}
                            >
                                {name.charAt(0)}
                            </div>
                        )}
                    </div>

                    {/* Badge */}
                    <div
                        style={{
                            background: 'rgba(248, 80, 50, 0.1)',
                            border: '1px solid rgba(248, 80, 50, 0.3)',
                            padding: '10px 24px',
                            borderRadius: 100,
                            color: '#F85032',
                            fontSize: 18,
                            fontWeight: 600,
                            letterSpacing: '0.1em',
                            textTransform: 'uppercase',
                            marginBottom: 24,
                            display: 'flex',
                            alignItems: 'center',
                            gap: 8
                        }}
                    >
                        {category}
                    </div>

                    {/* Name */}
                    <div
                        style={{
                            fontSize: 70,
                            fontWeight: 900,
                            color: 'white',
                            lineHeight: 1,
                            textAlign: 'center',
                            marginBottom: 16,
                            letterSpacing: '-0.02em',
                        }}
                    >
                        {name}
                    </div>

                    {/* Subtitle */}
                    <div
                        style={{
                            fontSize: 26,
                            color: '#94a3b8',
                            textAlign: 'center',
                            maxWidth: '700px',
                            lineHeight: 1.4,
                        }}
                    >
                        Verified Host on Tripzeo
                    </div>
                </div>

                {/* Top Right Logo */}
                <div style={{ position: 'absolute', top: 60, right: 60, display: 'flex', alignItems: 'center', gap: 14, zIndex: 20 }}>
                    <svg
                        version="1.1"
                        id="Layer_1"
                        xmlns="http://www.w3.org/2000/svg"
                        x="0px"
                        y="0px"
                        width="46"
                        height="42"
                        viewBox="228 256 567 514"
                        xmlSpace="preserve"
                    >
                        <path
                            fill="white"
                            opacity="1"
                            stroke="none"
                            d="M471.000061,256.659729 C525.999084,256.658356 580.498047,256.656921 634.997070,256.656342 C650.448181,256.656189 665.902283,256.460205 681.349731,256.697540 C724.916748,257.366974 768.155090,289.377594 776.383667,335.906372 C782.598206,371.047150 773.380737,401.467865 748.799072,427.407532 C732.681519,444.415588 716.014832,460.870605 699.546387,477.527832 C675.023865,502.331390 650.739380,527.370056 626.282654,552.238892 C609.984131,568.812073 593.529358,585.231384 577.203369,601.777588 C553.195129,626.109680 529.331665,650.585144 505.233246,674.827332 C484.739685,695.443237 465.732117,717.622742 443.236115,736.139465 C426.378754,750.014832 407.410828,760.108093 385.940033,765.689941 C369.337738,770.006165 352.518127,770.715149 335.974945,768.946167 C310.637939,766.236877 288.846313,755.016479 271.626801,735.638611 C252.866684,714.527039 245.453705,689.645020 247.818527,661.961792 C249.904358,637.544373 259.601410,616.321594 277.101868,598.737244 C292.215912,583.550720 306.776367,567.814453 321.813232,552.549438 C345.622131,528.379333 369.701874,504.476044 393.522705,480.317596 C416.382538,457.133820 438.987976,433.699158 461.839142,410.506744 C478.960175,393.130035 496.779449,376.410858 513.307068,358.494202 C520.475708,350.723114 527.753113,348.179565 537.988831,348.431152 C565.138611,349.098450 592.315613,348.644135 619.481689,348.692688 C621.547058,348.696381 623.778992,348.084534 625.927429,349.700867 C625.235962,352.055084 623.464600,353.561493 621.875488,355.165314 C584.654236,392.731384 547.365723,430.231140 510.215942,467.867767 C486.460022,491.935028 462.945496,516.240295 439.248566,540.365967 C423.508606,556.390747 407.574005,572.224365 391.841217,588.256104 C371.964935,608.510010 352.152161,628.826965 332.420227,649.221497 C323.902130,658.025635 322.189423,670.377625 326.945679,680.013916 C331.935638,690.123718 347.512482,695.633972 357.405151,694.057007 C381.671387,690.188538 399.435028,676.479248 415.772614,659.624390 C436.585052,638.153015 457.676971,616.952698 478.612244,595.600098 C508.750549,564.860901 538.784180,534.018799 568.999084,503.355225 C592.450684,479.555389 616.202209,456.051086 639.649841,432.247345 C657.823364,413.797760 675.925476,395.269714 693.693298,376.431396 C705.822327,363.571533 704.211060,344.100922 687.888123,334.919525 C684.457214,332.989685 680.711487,332.367859 676.887268,332.366852 C571.555908,332.339508 466.223053,332.022308 360.894043,332.519196 C322.058289,332.702423 296.616028,370.008087 307.764221,405.664124 C314.966675,428.700317 336.036621,444.465088 360.671509,444.655060 C373.170502,444.751434 385.671082,444.638275 398.170532,444.698181 C400.049408,444.707184 402.254059,443.978851 403.595978,446.163208 C403.639526,447.932953 402.278137,448.777832 401.249084,449.818481 C387.339111,463.885468 373.555634,478.081238 359.453247,491.952759 C351.999847,499.284088 344.171356,506.271606 337.486664,514.304016 C334.108124,518.363708 330.222015,518.110779 326.324188,516.937561 C306.930023,511.099579 289.049164,502.470337 273.884735,488.609802 C255.078918,471.420898 241.531662,451.117249 234.455109,426.465881 C230.340332,412.131897 227.816437,397.609344 228.839676,382.794312 C229.821091,368.584442 232.497086,354.643585 237.666367,341.144470 C248.414154,313.077606 266.555573,291.370544 292.024841,275.676819 C307.445435,266.174896 324.216278,260.350739 342.116302,257.611664 C346.080780,257.005035 350.006714,256.639740 354.008179,256.642853 C392.838806,256.673065 431.669434,256.660431 471.000061,256.659729 z"
                        />
                        <path
                            fill="white"
                            opacity="1"
                            stroke="none"
                            d="M631.402954,569.392090 C650.696594,549.903320 669.819824,530.728882 688.730530,511.347107 C691.740601,508.262024 694.624268,507.765900 698.473999,508.824036 C721.825317,515.242065 742.270325,526.657043 759.218384,544.059631 C778.869934,564.238159 790.780762,588.216064 795.477173,616.074280 C799.008240,637.019409 797.257080,657.443848 791.200806,677.512939 C786.656433,692.571960 778.939392,706.137573 769.413940,718.710999 C757.234741,734.787231 741.541260,746.548767 724.024719,755.992920 C713.343384,761.751892 701.758545,764.992676 689.839661,767.541199 C676.579224,770.376587 663.317932,769.240112 650.066772,769.294678 C600.427124,769.499268 550.786194,769.378479 501.145782,769.380188 C482.849670,769.380859 464.553131,769.312927 446.257751,769.428833 C440.956085,769.462402 435.660461,770.092590 430.356323,770.306274 C428.616272,770.376221 426.611145,770.866455 425.205231,768.718323 C425.886414,766.065063 428.550262,765.331787 430.518646,764.144958 C447.972748,753.620911 463.467621,740.747681 477.457764,725.954285 C486.459747,716.435364 495.820831,707.256958 504.903412,697.812927 C507.765076,694.837402 510.946777,693.615417 515.096313,693.623291 C564.255188,693.716980 613.414490,693.615234 662.573364,693.717651 C677.515686,693.748779 690.720520,689.323547 701.726624,679.198608 C721.468567,661.037048 727.241760,632.018494 712.188599,608.164185 C703.123901,593.799561 690.179199,585.726440 674.200928,581.759216 C671.673950,581.131775 668.907593,581.383301 666.251465,581.375549 C653.753540,581.339233 641.255310,581.375244 628.757385,581.328674 C626.827026,581.321472 624.406616,581.959595 623.326050,579.785522 C622.112549,577.344116 624.694641,576.386108 625.931213,575.005066 C627.596252,573.145569 629.411438,571.420532 631.402954,569.392090 z"
                        />
                    </svg>
                    <div style={{ color: 'white', fontSize: 32, fontWeight: 700, letterSpacing: '-0.05em', opacity: 0.9 }}>tripzeo</div>
                </div>
            </div>
        ),
        {
            ...size,
        }
    )
}
