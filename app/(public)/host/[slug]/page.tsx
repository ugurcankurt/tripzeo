...
import { MapPin, Calendar, ShieldCheck, Mail, Globe, Star } from "lucide-react"
...
export default async function HostProfilePage({ params }: { params: { slug: string } }) {
    const supabase = createClient()
    const { slug } = await params

    // Extract ID (last 36 chars)
    ...
    const { data: experiencesData } = await (await supabase)
        .from('experiences')
        .select('*')
        .eq('host_id', hostId)
        .eq('is_active', true)
    ...
    // A. Calculate stats directly from Experiences (Source of Truth)
    // This ensures what is shown on the cards matches the profile summary
    ...
    // B. Fetch all reviews for display
    // We use the ReviewList component now for consistency
        ...
        "breadcrumb": {
            ...
            "itemListElement": [
                ...
                },
                {
                    ...
                    "position": 2,
                    "name": profile.full_name,
                    "item": `https://tripzeo.com/host/${slug}`
                }
            ]
        }
    }
